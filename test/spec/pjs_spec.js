describe('identity', function() {
  ...
  it('should return whatever value is passed into it', function() {
    expect(pjs.identity(1)).to.equal(1);

    expect(pjs.identity('string')).to.equal('string');

    expect(pjs.identity(false)).to.equal(false);

    expect(pjs.identity(uniqueObject)).to.equal(uniqueObject);
  });
});
...
describe.skip('first', function() {
  ...
  it('should be able to accept a user-defined index', function() {
    expect(pjs.first([1,2,3], 0)).to.eql([]);
    expect(pjs.first([1,2,3], 2)).to.eql([1, 2]);
    expect(pjs.first([1,2,3], 5)).to.eql([1, 2, 3]);
  });
});
...
describe.skip('last', function() {
  ...
  it('should accept an index argument', function() {
    expect(pjs.last([1,2,3], 2)).to.eql([2, 3]);
  });

  it('should return nothing if zero is passed in as the index', function() {
    expect(pjs.last([1,2,3], 0)).to.eql([]);
  });

  it('should return all the array\'s elements if the index argument is larger than the length of the array', function() {
    expect(pjs.last([1,2,3], 5)).to.eql([1, 2, 3]);
  });
});
...
describe.skip('each', function() {
  it('should iterate over arrays, providing access to the element, index, and array itself', function() {
    ...
    pjs.each(animals, function(animal, index, list) {
  ...
  it('should iterate over objects, providing access to the element, index, and object itself', function() {
    ...
    pjs.each(animals, function(animal, key, object) {
...
describe.skip('indexOf', function() {
  it('should have 40 in the list', function() {
    ...
    expect(pjs.indexOf(numbers, 40)).to.be(3);
  });
...
describe.skip('filter', function() {
  ...
  it('should return all odd numbers in an array', function() {
    ...
    var odds = pjs.filter([1, 2, 3, 4, 5, 6], isOdd);
...
describe.skip('reject', function() {
  ...
  it('should reject all odd numbers', function() {
    var isOdd = function(num) { return num % 2 !== 0; };
    var evens = pjs.reject([1, 2, 3, 4, 5, 6], isOdd);
...
describe.skip('uniq', function() {
  it('should return all unique values contained in an unsorted array', function() {
    ...
    expect(pjs.uniq(list)).to.eql([1, 2, 3, 4]);
  });
  ...
  it('should handle iterators that work with a sorted array', function() {
    ...
    expect(pjs.uniq(list, true, iterator)).to.eql([1, 2, 3, 4]);
  });
});
...
describe.skip('pluck', function() {
  it('should return values contained at a user-defined property', function() {
    ...
    expect(pjs.pluck(people, 'name')).to.eql(['moe', 'curly']);
  });
});
...
describe.skip('invoke, when provided a function reference', function() {
  it('runs the input function on each item in the array, and returns a list of results', function() {
    ...
    var reversedStrings = pjs.invoke(['dog', 'cat'], reverse);
...
describe.skip('contains', function() {
  ...
  it('should return true if a collection contains a user-specified value', function() {
    expect(pjs.contains([  4,   5,   6], 5)).to.equal(true);
  });

  it('can operate on objects', function(){
    expect(pjs.contains({a:4, b:5, c:6}, 5)).to.equal(true);
  });
});
...
describe.skip('every', function() {
  ...
  it('passes for a collection of all-truthy results', function() {
    expect(pjs.every([true, {}, 1], pjs.identity)).to.equal(true);
  });

  it('fails for a collection of all-falsy results', function() {
    expect(pjs.every([null, 0, undefined], pjs.identity)).to.equal(false);
  });

  it('fails for a collection containing mixed falsy and truthy results', function() {
    expect(pjs.every([true, false, 1], pjs.identity)).to.equal(false);
  });

  it('handles callbacks that do work on the input', function() {
    var isEven = function(num) { return num % 2 === 0; };

    expect(pjs.every([0, 10, 28], isEven)).to.equal(true);
    expect(pjs.every([0, 11, 28], isEven)).to.equal(false);
  });

  it('casts the result to a boolean', function() {
    expect(pjs.every([1], pjs.identity)).to.equal(true);
    expect(pjs.every([0], pjs.identity)).to.equal(false);
  });

  it('treats each item as as a callback result when no callback is provided', function() {
    expect(pjs.every([true, true, true])).to.equal(true);
  });

  it('works when provded a collection containing undefined values', function() {
    expect(pjs.every([undefined, undefined, undefined], pjs.identity)).to.equal(false);
  });
});
...
describe.skip('some', function() {
  ...
  it('should handle the empty set', function() {
    expect(pjs.some([])).to.equal(false);
  });

  it('passes for a collection of all-truthy results', function() {
    expect(pjs.some([true, {}, 1], pjs.identity)).to.equal(true);
  });

  it('fails for a collection of all-falsy results', function() {
    expect(pjs.some([null, 0, undefined], pjs.identity)).to.equal(false);
  });

  it('passes for a collection containing mixed falsy and truthy results', function() {
    expect(pjs.some([true, false, 1], pjs.identity)).to.equal(true);
  });

  it('passes for a set containing one truthy value that is a string', function() {
    expect(pjs.some([null, 0, 'yes', false])).to.equal(true);
  });

  it('fails for a set containing no matching values', function() {
    expect(pjs.some([1, 11, 29], isEven)).to.equal(false);
  });

  it('passes for a collection containing one matching value', function() {
    expect(pjs.some([1, 10, 29], isEven)).to.equal(true);
  });

  it('casts the result to a boolean', function() {
    expect(pjs.some([1], pjs.identity)).to.equal(true);
    expect(pjs.some([0], pjs.identity)).to.equal(false);
  });
});
...
describe.skip('extend', function() {
  it('returns the first argument', function() {
    var to = {};
    var from = {};
    var extended = pjs.extend(to, from);
  ...
  it('should extend from multiple source objects', function() {
    var extended = pjs.extend({x:1}, {a:2}, {b:3});
  ...
  it('in the case of a conflict, it should use the last property\'s values when extending from multiple source objects', function() {
    var extended = pjs.extend({x:'x'}, {a:'a', x:2}, {a:1});

    expect(extended).to.eql({x:2, a:1});
  });

  it('should copy undefined values', function() {
    var extended = pjs.extend({}, {a: void 0, b: null});
...
describe.skip('defaults', function() {
  ...
  beforeEach(function() {
    ...
    pjs.defaults(options, {zero: 1, one: 10, twenty: 20}, {empty: 'full'}, {nan: 'nan'}, {word: 'word'}, {word: 'dog'});
  });

  it('returns the first argument', function() {
    var to = {};
    var from = {};
    var defaulted = pjs.defaults(to, from);
  ...
  it('prefers the first value found, when two objects are provided with properties at the same key', function() {
    ...
    var from = {a: 1};
    var alsoFrom = {a: 2};
    var defaulted = pjs.defaults(to, from, alsoFrom);
...
describe.skip('sortBy', function() {
  ...
  it('should handle undefined values', function() {
    ...
    var result = pjs.sortBy(list, function(i) { return i; });
  ...
  it('should sort by length', function() {
    ...
    var sorted = pjs.sortBy(list, 'length');
  ...
  it('should produce results that change the order of the list as little as possible', function() {
    ...
    var actual = pjs.sortBy(collection, function(pair) {
      return pair.x;
    });
...
describe.skip('flatten', function() {
  it('can flatten nested arrays', function() {
    ...
    expect(pjs.flatten(nestedArray)).to.eql([1,2,3,4]);
  });
});
...
describe.skip('zip', function() {
  it('should zip together arrays of different lengths', function() {
    ...
    expect(pjs.zip(names, ages, leaders)).to.eql([
      ['moe', 30, true],
      ['larry', 40, undefined],
      ['curly', 50, undefined]
    ]);
...
describe.skip('intersection', function() {
  it('should take the set intersection of two arrays', function() {
    ...
    expect(pjs.intersection(stooges, leaders)).to.eql(['moe']);
  });
});
...
describe.skip('difference', function() {
  ...
  it('should return the difference between three arrays', function() {
    var result = pjs.difference([1, 2, 3, 4], [2, 30, 40], [1, 11, 111]);
...
describe.skip('throttle', function() {
  it('throttled functions should only be able to be called again after the specified time', function(done) {
    ...
    var throttledIncr = pjs.throttle(incr, 32);
    throttledIncr();
    throttledIncr();
  ...
  it('throttled functions called repeatedly should adhere to time limitations', function(done) {
    ...
    var throttledIncr = pjs.throttle(incr, 64);
    var results = [];
